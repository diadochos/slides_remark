...
\end{split}
$$
を調べる．これらは漸近的には0に近づく．これは(サンプルの分布による積分ひいては)$\hat w\_n$という特異点を持つ集合($W\_0$)に確率収束をしていくような確率変数の分布での積分になっている．(check → @todo)
$\hat w\_n$自身は分布収束しないので，
特異点を正規交差型にした方のパラメーター空間で$\xi\_n(u) := \sum\_{i=1}^n \frac{K(w) - f(X\_i, w)}{\sqrt{nK(w)}}$を調べる．
